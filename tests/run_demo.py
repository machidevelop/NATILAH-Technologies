...
from qstrainer.quantum import PipelineConfig, QuantumAdvantagePipeline
...
class DatacenterJobGenerator:
    ...
    def generate_job_batch(self, batch_size: int, base_step: int = 0) -> list[ComputeTask]:
        ...
        for i in range(batch_size):
            ...
            if conv_rate > 0:
                loss = max(
                    profile["initial_loss"] * math.exp(-conv_rate * step) + self.rng.gauss(0, 0.05),
                    0.01,
                )
    ...
    def generate_redundant_burst(
        self,
        count: int,
        profile_name: str = "finetune_bert",
    ) -> list[ComputeTask]:
...
def estimate_cost_saved(task: ComputeTask) -> float:
    ...
    rate = 2.50 if gpu_info is None else gpu_info["cost_per_hr"]
...
def main():
    ...
    print(
        f"  Tasks processed: 1,000 across {len(warmup_gpus_used)} GPUs"
        f" / {len(warmup_nodes_used)} nodes"
    )
    print("  Workload mix:")
    for model, count in sorted(warmup_profiles.items(), key=lambda x: -x[1]):
        profile_info = next(
            (v for v in WORKLOAD_PROFILES.values() if v["model_name"] == model), None
        )
    ...
    print(
        f"  Total feature matrix: {len(warmup_tasks):,} tasks x {len(derived)} features "
    ...
    print("  Injected:  200 converged BERT fine-tuning tasks")
    print(
        f"  Detected:  {len(redundant_decisions)} as redundant "
    ...
    print(
        f"  Total tasks:  {len(mixed_tasks):,} (2,000 mixed + 300 converged FT + 100 converged LLM)"
    )
    ...
    print(
        f"  Throughput:   {len(mixed_tasks) / mixed_wall:,.0f} tasks/sec  ({mixed_wall:.3f}s wall)"
    )
    ...
    qubo_cost_saved = sum(
        estimate_cost_saved(qubo_batch[i])
    ...
    print("  Batch size:    64 (48 productive + 16 redundant)")
    ...
    qubo2_cost_saved = sum(
        estimate_cost_saved(qubo_llm_batch[i])
        for i, r in enumerate(qubo_llm_results)
        if r.verdict != TaskVerdict.EXECUTE
    )

    print("  Batch size:    48 (36 mixed + 12 converged LLM)")
    ...
    qa_pipeline = QuantumAdvantagePipeline(
        PipelineConfig(
            p_layers=2,
            n_restarts=3,
            maxfev=80,
    ...
    print(
        f"  Conflict graph:  {qa_result.original_edges} edges "
    ...
    print(f"  Edges dropped:   {qa_result.edges_dropped} ({qa_result.edge_drop_ratio:.0%})")
    print(
        f"  Purified graph:  {qa_result.purified_edges} edges "
    ...
    print(
        f"  Coloring valid:  original={qa_result.original_coloring_valid}, "
    ...
    qa2_pipeline = QuantumAdvantagePipeline(
        PipelineConfig(
            p_layers=2,
            n_restarts=3,
            maxfev=80,
    ...
    print(
        f"  Conflict graph:  {qa2_result.original_edges} edges "
    ...
    print(f"  Edges dropped:   {qa2_result.edges_dropped} ({qa2_result.edge_drop_ratio:.0%})")
    print(
        f"  Purified graph:  {qa2_result.purified_edges} edges "
    ...
    combined_cost = warmup_cost_saved + redundant_cost + mixed_cost_saved + all_qubo_cost
    ...
    print(f"  {'─' * 46}")
    print("  Pipeline stages:  Redundancy -> Convergence -> Predictive")
    print(f"  QUBO scheduler:   64x64 + 48x48 via {solver_name}")
    print(
        f"  Quantum advantage (16 tasks): makespan {qa_result.original_makespan} -> "
        f"{qa_result.purified_makespan} ({qa_result.makespan_reduction:.0%} reduction)"
    )
    print(
        f"  Quantum advantage (18 tasks): makespan {qa2_result.original_makespan} -> "
    ...
    results = {
        ...
        "cluster": {
            ...
            "layout": [{"node": n, "model": m, "count": c} for n, m, c in CLUSTER_LAYOUT],
