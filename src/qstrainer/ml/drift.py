...
class DriftDetector:
    ...
    def __init__(
        ...
        self._baseline: np.ndarray | None = None  # (n_baseline, n_features)
        self._recent: list[np.ndarray] = []  # ring of recent vectors
        self._n_since_baseline: int = 0

        # Page–Hinkley accumulators (per feature)
        self._ph_sum: np.ndarray | None = None
        self._ph_mean: np.ndarray | None = None
        self._ph_min: np.ndarray | None = None
    ...
    def check(self, feature_names: list[str] | None = None) -> DriftReport:
        ...
        for i in range(n_features):
            psi = self._compute_psi(self._baseline[:, i], recent_matrix[:, i])
    ...
    def _update_page_hinkley(self, vec: np.ndarray) -> None:
        ...
        assert self._ph_min is not None
...
class OnlineRetrainer:
    ...
    def observe(self, feature_vector: np.ndarray, is_healthy: bool = True) -> DriftReport | None:
    ...
    def get_retrain_data(self) -> np.ndarray | None:
